...
export default function Slider({ className, name, value, min, max, step, onChange }: SliderProps) {
    ...
    const onInput = (event: React.ChangeEvent<HTMLInputElement>, checkBounds = true) => {
    ...
    return (
        <div className="flex justify-center gap-x-2 mx-auto my-[2px]">
            ...
            <span className="grow-0 shrink basis-[150px] text-left"><input className="border-2 border-black border-solid rounded-md w-11 hover:bg-gray-100 focus:bg-white transition-colors" id={`${className}-text`} onInput={e => onInput(e as React.ChangeEvent<HTMLInputElement>, false)} onFocus={onCapture} onBlur={onRelease} type="text" value={val} /></span>
        </div>
    );
}