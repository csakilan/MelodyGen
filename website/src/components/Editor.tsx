...
import { GRID_ROWS, GRID_SIZE_X, GRID_SIZE_Y, LOWEST_OCTAVE, MAX_MEASURES, MIN_MEASURES, NOTE_COUNT, PIANO_KEY_GAP, PIANO_KEY_WIDTH, QUARTER_SUBDIVISIONS } from "@/util/config";
...
export default function Editor() {
  ...
  const onTestNote = (id: number, shouldRelease: boolean = false) => {
    ...
    if (shouldRelease) {}
      ...
      synth.current?.triggerAttackRelease(convertNote({position: [0, id], duration: 4}).note, "3n");
    Tone.getTransport().start();
  };
  ...
  return (
    <div>
      <div ref={backgroundRef} className="m-auto overflow-x-auto" style={{maxWidth: 8 * GRID_SIZE_X * QUARTER_SUBDIVISIONS * 4 + PIANO_KEY_WIDTH + PIANO_KEY_GAP + 8}}>
        <EditorBackground musicKey={key} octave={octave} verticalLinesPerQuarter={verticalLinesPerQuarter} verticalLinesPerWhole={verticalLinesPerWhole} verticalLineBeatInterval={verticalLineBeatInterval} notes={
          <div className="relative w-full h-full">
            { notes.current.map((note, i) => <Note key={i} position={note.position} duration={note.duration} gridMultiplier={gridMultiplier} lastNote={i == notes.current.length - 1} EDITOR_WIDTH={EDITOR_WIDTH} updateNote={getUpdateNoteHandler(i)} />) }
          </div>
        } EDITOR_WIDTH={EDITOR_WIDTH} MAX_GRID_X={MAX_GRID_X} onTestNote={onTestNote} />
      </div>
      <div className="m-auto w-[80%]">
        ...
        <div className="grid gap-y-4">
          <div className="flex flex-row gap-x-4 justify-center items-center">
            ...
            <Button onClick={() => setMeasureCount(i => Math.min(MAX_MEASURES, i + 1))}>Add Measure</Button>
            <Button onClick={() => setMeasureCount(i => Math.max(MIN_MEASURES, i - 1))}>Remove Measure</Button>
          </div>
          <Slider className="bpm" name="BPM" value={bpm} min={60} max={240} step={1} onChange={setBpm} />
          <div className="flex flex-row gap-x-4 justify-center items-center">
            <label htmlFor="grid-subdivision">Grid</label>
            <select id="grid-subdivision" value={gridMultiplier} onChange={(e) => setGridMultiplier(parseInt(e.target.value))}>
            ...
            <select id="time-signature" value={quartersPerBar} onChange={(e) => setQuartersPerBar(parseInt(e.target.value))}>
              <option value={3}>3/4</option>
              <option value={4}>4/4</option>
            </select>
            <label htmlFor="key-signature">Key</label>
            <select id="key-signature" value={key} onChange={(e) => setKey(parseInt(e.target.value))}>
              {
                Array(12).fill(0).map((_, i) => 11 - i).map(i => <option key={i} value={i}>{noteNames[i]}</option>)
              }
            </select>
            <label htmlFor="octave">Octave</label>
            <select id="octave" value={octave} onChange={(e) => setOctave(parseInt(e.target.value))}>
              {
                Array(3).fill(0).map((_, i) => 2 - i + LOWEST_OCTAVE).map(i => <option key={i} value={i}>{i}</option>)
              }
            </select>
          </div>
          <div className="flex flex-row gap-x-4 justify-center items-center">
